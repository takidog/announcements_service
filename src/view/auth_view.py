...
class Login:
    ...
    def on_post(self, req, resp):
        ...
        if (
            isinstance(req_json.get("fcmToken"), str)
            and len(req_json.get("fcmToken", "")) > 200
        ):
            raise falcon.HTTPBadRequest(description="FCM key too long.")
        if (
            not isinstance(req_json.get("fcmToken"), str)
            and req_json.get("fcmToken") is not None
        ):
            raise falcon.HTTPBadRequest(description="FCM key error. (typeError)")
        # 401 error will raise in auth_service
        login_jwt = self.auth_service.login(
            username=req_json["username"],
            password=req_json["password"],
            fcm_token=req_json.get("fcmToken"),
        )
        resp.set_cookie("Authorization", f"Bearer {login_jwt}", max_age=JWT_EXPIRE_TIME)
...
class Register:
    ...
    def on_post(self, req, resp):
        ...
        if register_status:
            ...
            resp.set_cookie(
                "Authorization", f"Bearer {login_jwt}", max_age=JWT_EXPIRE_TIME
            )
            resp.media = {"key": login_jwt}
            resp.status = falcon.HTTP_200
            return True
...
class AppleSignInByIdToken:
    ...
    def on_post(self, req, resp):
        ...
        login_jwt = self.auth_service.apple_sign_in_by_id_token(
            ...
            bundle_id=req_json.get("bundleId"),
            fcm_token=req_json.get("fcmToken"),
        )
        resp.set_cookie("Authorization", f"Bearer {login_jwt}", max_age=JWT_EXPIRE_TIME)
        resp.media = {"key": login_jwt}
